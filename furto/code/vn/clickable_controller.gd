...
const CURSOR: GDScript = preload("res://code/vn/cursor_root.gd")

@export var clickable: bool = true: set = set_clickable
@export var cursor: CURSOR.Type: set = set_cursor_type
...
func set_cursor_type(cursor_type: CURSOR.Type) -> void:
	cursor = cursor_type
